...
uint32_t OrderManager::buy(const char *order_template, double quantity,
                           double limit_price) {
  return create_order(order_template, roq::TradeDirection::Buy,
                      quantity, limit_price);
}

uint32_t OrderManager::sell(const char *order_template, double quantity,
                            double limit_price) {
  return create_order(order_template, roq::TradeDirection::Sell,
                      quantity, limit_price);
}
...
void OrderManager::on(const roq::TimerEvent& event) {
...
void OrderManager::on(const roq::ModifyOrderAckEvent& event) {
  check(event.message_info);
...
void OrderManager::on(const roq::OrderUpdateEvent& event) {
  ...
  switch (order_update.status) {
    ...
    case roq::OrderStatus::Cancelled: {
...
void OrderManager::check(const roq::MessageInfo& message_info) {

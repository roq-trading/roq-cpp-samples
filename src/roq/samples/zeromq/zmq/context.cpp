...
void Context::set(int option_name, int option_value) {
  ...
  if (result == -1)
    throw SystemError{std::make_error_code(static_cast<std::errc>(errno)), "zmq_ctx_set"sv};
  assert(result == 0);
}
...
int Context::get(int option_name) {
  ...
  if (result == -1)
    throw SystemError{std::make_error_code(static_cast<std::errc>(errno)), "zmq_ctx_get"sv};
  assert(result >= 0);
  return result;
}
